...
public class AfaInvoiceReportValidationTest {
	...
	@BeforeSuite
	@Parameters({"db_user","db_password","jdbc_url"})
	public void getConnection(String db_user,String db_password,String jdbc_url) {
		AutomationStringUtilities decoder = new AutomationStringUtilities();
		conn.setUsername(decoder.decryptValue(db_user).trim());
		conn.setPassword(decoder.decryptValue(db_password).trim());
		conn.setUrl(jdbc_url);
